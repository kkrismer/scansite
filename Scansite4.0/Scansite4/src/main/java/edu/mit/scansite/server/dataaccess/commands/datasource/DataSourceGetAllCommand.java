...
public class DataSourceGetAllCommand extends DbQueryCommand<List<DataSource>> {
		...
		try {
			while (result.next()) {
				...
				dataSources.add(new DataSource(result.getInt(c
						.getcDataSourcesId()), type, identifierType, result.getString(c
						.getcDataSourcesShortName()), result.getString(c
						.getcDataSourcesDisplayName()), result.getString(c
						.getcDataSourcesDescription()), result.getString(c
						.getcDataSourcesVersion()), result.getDate(c
						.getcDataSourcesLastUpdate()), result
	...
	protected String doGetSqlStatement() throws DataAccessException {
		// SELECT * FROM `datasources` INNER JOIN datasourcetypes USING (dataSourceTypesId) INNER JOIN identifiertypes USING (identifierTypesId)
		...
		sql.append(CommandConstants.SELECT).append('*')
				...
				.append(CommandConstants.RPAR).append(CommandConstants.INNERJOIN)
		...
		if (type != null) {
			sql.append(CommandConstants.WHERE)
					.append(c.getcDataSourceTypesId()).append(" = ")
					.append(type.getId());
		}
		sql.append(CommandConstants.ORDERBY).append(
				c.getcDataSourcesDisplayName());
		return sql.toString();
	}
}
