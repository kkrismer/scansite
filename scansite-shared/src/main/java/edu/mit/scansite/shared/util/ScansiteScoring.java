...
public class ScansiteScoring {
     ...
     *
     * @param score       A sequence's actual score.
     * @param totalNumber The total number of entries displayed in the histogram
    ...
    public double getHistogramScore(double score, int totalNumber) {
        ...
        if (totalNumber < 1000) {
            ...
            SWS = ScansiteConstants.HIST_SCORE_MAX / (Math.sqrt(totalNumber));
            // for less bins: divide by
            // ((2 * ScansiteAlgorithms.log2(totalNumber) + 1);
        }
     ...
     *
     * @param position The position within the sequence.
     * @param sequence The sequence as character array.
     * @param motif    The motif that is used for calculating the score.
        ...
        for (int windowPosition = 0; windowPosition < ScansiteConstants.WINDOW_SIZE; ++windowPosition) {
            ...
            if (!AminoAcid.isAa(currentAa)) {
                ...
                double currentMatrixValue = motif.getValue(currentAa,
                        windowPosition);
                if (currentMatrixValue != ScansiteConstants.FIXED_SITE_SCORE) {
                    // Don't score the fixed residue(s)
                    if (currentMatrixValue == 0) {
                        // typically, "0" values in matrices occur at the
                        // terminals (in rows that are not all-zero).
                        // this indicates that this residue is invalid in this position.
     ...
     *
     * @param m The motif that's optimal score is calculated and set.
     */
    public void calculateOptimalScore(Motif m) {
        ...
        // Calculate optimal scores for sites that contain terminals considering only sites that
        // contain all the fixed residues. The loop also calculates the full window score.
        // Compare score to the current optimal score (for a full window) and maintain the highest score.
        for (int i = -1; i < firstFixedIndex; ++i) {// N-terminal must be before
            // the first fixed residue
            for (int j = lastFixedIndex + 1; j <= WINDOW_SIZE; ++j) {
                // C-terminal must be after the last fixed residue calculate
                // the optimal score of a site with N-terminal at i and C-terminal at j
                ...
                if (i > -1) { // window starts with a N-terminal
                    log2Sum += ScansiteAlgorithms.log2(m.getValue(AminoAcid._N, i));
                    ++deg; // the terminal score is included in the average
                }

                // add max values within the current window to logsum
                for (int k = i + 1; k <= j - 1; ++k) {
                    if (isFixedSite(rowMax[k])) { // a fixed residue decrement number of degenerate positions average.
                        --deg;
                    } else if (rowMax[k] == 0) { // all row is zeroes: do not include it in the
                        ++zeroCount;
                ...
                if (j < WINDOW_SIZE) { // window ends with a C-terminal
                    log2Sum += ScansiteAlgorithms.log2(m.getValue(AminoAcid._C, j));
                    ++deg; // the terminal score is included in the average
                }

                // calculate score and keep the highest score
                double score = log2Sum / (deg - (double) zeroCount); // optimal score for the current window
                if (score > optScore) {
                    optScore = score;
                }
            }
     ...
     *
     * @param d The motif site's value.
     ...
     *
     * @param motif    A motif.
     * @param proteins A list of proteins protein.
     * @param maxScore A maximum score.
     ...
     * @param proteins        A list of proteins protein.
     * @param motifThresholds A maximum score.
     * @return Returns a list of hits.
     */
     ...
     *
     * @param motifs   A set of motifs.
     * @param p        A protein.
     * @param maxScore A maximum score. All results with a score >maxScore will be
     *                 discarded.
     * @return Returns a list of hits.
     */
     ...
     * @param motif    A motif.
     * @param p        A protein.
     * @param maxScore A maximum threshold score.
     * @return Returns a list of hits.
     */
    public ArrayList<ScanResultSite> scoreProtein(Motif motif,
        ...
        if(fixedCenters != null && fixedCenters.contains('K')) {
            fixedCenters.add('l');// only phosphorylation sites are expected

                    // however, for possible future changes all currently available modifications
                    // are covered by this approach (s,t,y,r,k,l)
        }
        // if the site was reported, the modified site was changed to a lower case letter
        ...
        if (isProteinScorable(seq)) {
            ...
            for (int i = 0; i < sequence.length; ++i) {
                if (fixedCenters.contains(sequence[i])
                        || fixedCenters.contains(Character.toLowerCase(sequence[i]))) {
                    hits.addAll(scoreSequencePermutations(i, sequence, motif, maxScore, p));
                }
            }
        }
        return hits;
    ...
    private ArrayList<ScanResultSite> scoreSequencePermutations(int i,
            char[] sequence, Motif motif, double maxScore, LightWeightProtein protein) {

        ArrayList<ScanResultSite> hits = new ArrayList<>();
        ArrayList<Character> modifiedResidues = getModifiedResidues();
        List<Integer> possibleModifiedResiduePositions = new ArrayList<>();
        ...
        for (int j = i - 7; j < i + 7; j++) {
            ...
            if (modifiedResidues.contains(sequence[j])) {
                if (influencesScoring(motif, sequence[j])) {
                    possibleModifiedResiduePositions.add(j);
                } else {
                    sequence = removePotentialModification(sequence, sequence[j]);
                }
            }
        }

        hits.addAll(scoreSequencePermutationRecursively(i, sequence, motif,
                possibleModifiedResiduePositions, 0, maxScore, protein));

        return hits;
    }

    private boolean influencesScoring(Motif motif, char modifiedResidue) {
        char residue;
        if (modifiedResidue == 's') {
            residue = 'S';
        } else if (modifiedResidue == 't') {
            residue = 'T';
        } else if (modifiedResidue == 'y') {
            residue = 'Y';
        } else if (modifiedResidue == 'r') {
            residue = 'R';
        } else if (modifiedResidue == 'k' || modifiedResidue == 'l') {
            residue = 'K';
        } else {
            return false;
        }
        ...
        for (int i=0; i < 15; i++) {
            double residueScore = motif.getValue(residue, i);
            double modifiedResidueScore = motif.getValue(modifiedResidue, i);
            if(residueScore != modifiedResidueScore) {
                areDifferent = true;
            }
        }
    ...
    private char[] removePotentialModification(char[] sequence, char modification) {
        for (int i=0; i < sequence.length; i++) {
            if (sequence[i] == modification) {
                if (modification == 'l') {
                    sequence[i] = 'K';
                } else {
    ...
    private ArrayList<ScanResultSite> scoreSequencePermutationRecursively(
            int fixedSite, char[] sequence, Motif motif,
            List<Integer> possibleModifiedResiduePositions,
            int modResiduePosIdx, double maxScore, LightWeightProtein protein) {

        ArrayList<ScanResultSite> results = new ArrayList<>();
        if(modResiduePosIdx == possibleModifiedResiduePositions.size()) { //also covers for no modifications at all
            double score = calculateSiteScore(fixedSite, sequence, motif);
            protein.setSequence(new String(sequence));
            ScanResultSite resultSite = processScore(score, maxScore, motif, protein, fixedSite);
            if (resultSite != null) {
        ...
        results.addAll(scoreSequencePermutationRecursively(fixedSite, sequence, motif,
                possibleModifiedResiduePositions, modResiduePosIdx + 1, maxScore, protein));

        results.addAll(scoreSequencePermutationRecursively(fixedSite, nonModifiedSequence, motif,
                possibleModifiedResiduePositions, modResiduePosIdx + 1, maxScore, protein));

        return results;
    }

    private ScanResultSite processScore(double score,
           double maxScore, Motif motif, LightWeightProtein protein, int fixedSite) {

        if (score >= 0.0 && score <= maxScore) {
            ++siteCount;
            return new ScanResultSite(motif, protein, null, null, fixedSite, roundScore(score));
        }
        return null;
    }

    private ArrayList<Character> getModifiedResidues() {
        ArrayList<Character> modifiedResidues = new ArrayList<>();
        modifiedResidues.add('s');
        modifiedResidues.add('t');
        modifiedResidues.add('y');
        modifiedResidues.add('a');
        modifiedResidues.add('k');
        modifiedResidues.add('l'); //although l is not yet in the sequence due to evidence data structure (can not distinguish different modifications)
     ...
     *
     * @param motif        A motif.
     * @param scoreToCount A map that maps scores to counts.
     * @param p            A single protein with a sequence set.
     * @return A map that maps the calculated scores to the number of hits with
     * exactly this score.
     */
