...
public class NewsGetCommand extends DbQueryCommand<NewsEntry> {
	...
	public NewsGetCommand(Properties dbAccessConfig, Properties dbConstantsConfig, int id) {
		super(dbAccessConfig, dbConstantsConfig);
		this.id = id;
	}

	@Override
	protected NewsEntry doProcessResults(ResultSet result) throws DataAccessException {
		try {
			if (result.next()) {
				...
				return new NewsEntry(result.getInt(c.getcNewsId()), result.getString(c.getcNewsTitle()),
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(c.SELECT).append(c.getcNewsId()).append(c.COMMA).append(c.getcNewsTitle()).append(c.COMMA)
				...
				.append(c.getcUsersEmail()).append(CommandConstants.RPAR).append(c.WHERE).append(c.getcNewsId())
				.append(c.EQ).append(id);
		return sql.toString();
	}
}
