...
public class MotifDataUpdateCommand extends DbUpdateCommand {
	...
	public MotifDataUpdateCommand(Properties dbAccessConfig, Properties dbConstantsConfig, Motif motif, int position) {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		for (AminoAcid aa : aas) {
			if (!aa.equals(AminoAcid._C) && !aa.equals(AminoAcid._N)) {
				sql.append(c.getcMatrixDataScorePrefix())
						...
						.append(CommandConstants.EQ).append(motif.getValue(aa, position))
						.append(CommandConstants.COMMA);
			}
		}
		sql.append(c.getcMatrixDataScoreEnd()).append(CommandConstants.EQ)
				.append(motif.getValue(AminoAcid._C, position)).append(CommandConstants.COMMA);
		...
		sql.append(c.getcMotifsId()).append(CommandConstants.EQ).append(motif.getId()).append(CommandConstants.AND);
		sql.append(c.getcMatrixDataPosition()).append(CommandConstants.EQ).append(position);
		return sql.toString();
	}
