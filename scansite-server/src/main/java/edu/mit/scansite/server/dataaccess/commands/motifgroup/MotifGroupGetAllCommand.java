...
public class MotifGroupGetAllCommand extends DbQueryCommand<List<MotifGroup>> {
	...
	public MotifGroupGetAllCommand(Properties dbAccessConfig, Properties dbConstantsConfig, MotifClass motifClass,
			boolean publicOnly) {
		super(dbAccessConfig, dbConstantsConfig);
	...
	@Override
	protected List<MotifGroup> doProcessResults(ResultSet result) throws DataAccessException {
		...
		try {
			while (result.next()) {
				...
				if (groups.containsKey(result.getInt(c.getcMotifGroupsId()))) {
					...
					group = new MotifGroup(result.getInt(c.getcMotifGroupsId()),
							result.getString(c.getcMotifGroupsDisplayName()),
							result.getString(c.getcMotifGroupsShortName()));
					groups.put(group.getId(), group);
				}
				group.getMotifs().add(new LightWeightMotif(result.getInt(c.getcMotifsId()),
						result.getString(c.getcMotifsDisplayName()), result.getString(c.getcMotifsShortName())));
			}
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(CommandConstants.SELECT).append('*').append(CommandConstants.FROM).append(c.gettMotifs())
				.append(CommandConstants.INNERJOIN).append(c.gettMotifGroups()).append(CommandConstants.USING)
				.append(CommandConstants.LPAR).append(c.getcMotifGroupsId()).append(CommandConstants.RPAR);
		if (motifClass != null) {
			...
			if (publicOnly) {
				sql.append(CommandConstants.WHERE).append(c.getcMotifsIsPublic()).append(CommandConstants.EQ)
						.append('1');
			}
		}
		return sql.toString();
