...
public class MotifGetCommand extends DbQueryCommand<Motif> {
	...
	public MotifGetCommand(Properties dbAccessConfig, Properties dbConstantsConfig, String shortName) {
		super(dbAccessConfig, dbConstantsConfig);
		this.shortName = shortName;
	}

	public MotifGetCommand(Properties dbAccessConfig, Properties dbConstantsConfig, int motifId) {
		super(dbAccessConfig, dbConstantsConfig);
		this.motifId = motifId;
	}

	@Override
	protected Motif doProcessResults(ResultSet result) throws DataAccessException {
		try {
			Motif m = null;
			int motifId = -1;
			...
			while (result.next()) {
				if (motifId == -1) {
					...
					currentIdentifier = new Identifier(result.getString(c.getcMotifIdentifierMappingIdentifier()),
							new IdentifierType(result.getInt(c.getcIdentifierTypesId()),
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(CommandConstants.SELECT).append(c.getcMotifsId()).append(CommandConstants.COMMA)
				...
				.append(c.getcMotifsMotifClass()).append(CommandConstants.COMMA).append(c.getcMatrixDataPosition())
				.append(CommandConstants.COMMA).append(c.getcMatrixDataScoreEnd()).append(CommandConstants.COMMA)
				.append(c.getcMatrixDataScoreStart()).append(CommandConstants.COMMA)
				.append(c.getcMotifIdentifierMappingIdentifier()).append(CommandConstants.COMMA)
				.append(c.getcIdentifierTypesId()).append(CommandConstants.COMMA).append(c.getcIdentifierTypesName());
		for (AminoAcid aa : aas) {
			if (!aa.equals(AminoAcid._C) && !aa.equals(AminoAcid._N)) {
		...
		sql.append(CommandConstants.FROM).append(c.gettMotifs()).append(CommandConstants.INNERJOIN)
				...
				.append(c.getcMotifsId()).append(CommandConstants.RPAR).append(CommandConstants.INNERJOIN)
				.append(c.gettMotifIdentifierMapping()).append(CommandConstants.USING).append(CommandConstants.LPAR)
				.append(c.getcMotifsId()).append(CommandConstants.RPAR).append(CommandConstants.INNERJOIN)
				.append(c.gettIdentifierTypes()).append(CommandConstants.USING).append(CommandConstants.LPAR)
				.append(c.getcIdentifierTypesId()).append(CommandConstants.RPAR).append(CommandConstants.WHERE);
		if (motifId == -1) {
		...
		sql.append(CommandConstants.ORDERBY).append(c.getcMotifsId()).append(CommandConstants.COMMA)
