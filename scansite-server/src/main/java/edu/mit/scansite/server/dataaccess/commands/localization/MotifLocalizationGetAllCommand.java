...
public class MotifLocalizationGetAllCommand extends DbQueryCommand<Map<Motif, LightWeightLocalization>> {
	...
	public MotifLocalizationGetAllCommand(Properties dbAccessConfig, Properties dbConstantsConfig,
	...
	@Override
	protected Map<Motif, LightWeightLocalization> doProcessResults(ResultSet result) throws DataAccessException {
		...
		try {
			while (result.next()) {
				...
				LocalizationType type = new LocalizationType(result.getInt(c.getcLocalizationTypesId()),
				...
				List<Motif> motifs = identifierToMotifsLUT.get(result.getString(c.getcProteinsIdentifier()));
				for (Motif motif : motifs) {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(CommandConstants.SELECT).append('*').append(CommandConstants.FROM)
				.append(c.gettLocalization(localizationDataSource)).append(CommandConstants.INNERJOIN)
				.append(c.gettLocalizationTypes()).append(CommandConstants.USING).append(CommandConstants.LPAR)
				.append(c.getcLocalizationTypesId()).append(CommandConstants.RPAR).append(CommandConstants.WHERE);
		sql.append(c.getcProteinsIdentifier()).append(CommandConstants.IN).append(CommandConstants.LPAR);
		List<String> identifiers = new ArrayList<>(identifierToMotifsLUT.keySet());
