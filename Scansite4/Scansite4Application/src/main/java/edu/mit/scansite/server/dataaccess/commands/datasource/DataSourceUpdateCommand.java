...
public class DataSourceUpdateCommand extends DbUpdateCommand {
	...
	public DataSourceUpdateCommand(Properties dbAccessConfig, Properties dbConstantsConfig, DataSource dataSource) {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(c.getcDataSourceTypesId()).append(CommandConstants.EQ).append(dataSource.getType().getId())
				.append(CommandConstants.COMMA);
		sql.append(c.getcIdentifierTypesId()).append(CommandConstants.EQ).append(dataSource.getIdentifierType().getId())
				.append(CommandConstants.COMMA);
		sql.append(c.getcDataSourcesDescription()).append(CommandConstants.EQ)
				.append(CommandConstants.enquote(dataSource.getDescription())).append(CommandConstants.COMMA);
		sql.append(c.getcDataSourcesDisplayName()).append(CommandConstants.EQ)
				.append(CommandConstants.enquote(dataSource.getDisplayName())).append(CommandConstants.COMMA);
		sql.append(c.getcDataSourcesShortName()).append(CommandConstants.EQ)
				.append(CommandConstants.enquote(dataSource.getShortName())).append(CommandConstants.COMMA);
		sql.append(c.getcDataSourcesIsPrimary()).append(CommandConstants.EQ)
				.append(dataSource.isPrimaryDataSource() ? 1 : 0).append(CommandConstants.COMMA);
