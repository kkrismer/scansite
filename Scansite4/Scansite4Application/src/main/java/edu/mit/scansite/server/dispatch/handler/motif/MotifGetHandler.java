	...
	public MotifGetResult execute(MotifGetAction action,
		...
		try {
			...
			if (m != null) {
				...
				MotifLogoPainter logoPainter = new MotifLogoPainter(m, false);
				...
				// toggled
				MotifLogoPainter toggleLogoPainter = new MotifLogoPainter(m, true);
				String toggleMotifLogoUrl = motifLogoUrl + "toggled";
				ImageInOut tiio = new ImageInOut();
				tiio.saveImage(toggleLogoPainter.getBufferedImage(), toggleMotifLogoUrl);

				String toggleClientFilePath = toggleMotifLogoUrl.replace(contextProvider.get().getRealPath("/"), "");
				if(toggleClientFilePath.startsWith("/") || toggleClientFilePath.startsWith("\\")) {
					toggleClientFilePath = toggleClientFilePath.substring(1);
				}

				return new MotifGetResult(new LightWeightMotif(m),
						clientFilePath, toggleClientFilePath, m.getIdentifiers(),
						m.getGroup(), m.getMotifClass());
			} else {
