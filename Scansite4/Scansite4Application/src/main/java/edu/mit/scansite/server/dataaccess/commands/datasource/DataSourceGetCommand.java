...
public class DataSourceGetCommand extends DbQueryCommand<DataSource> {
	...
	public DataSourceGetCommand(Properties dbAccessConfig, Properties dbConstantsConfig, int id) {
		super(dbAccessConfig, dbConstantsConfig);
		this.id = id;
	}

	public DataSourceGetCommand(Properties dbAccessConfig, Properties dbConstantsConfig, String shortName) {
		super(dbAccessConfig, dbConstantsConfig);
		this.shortName = shortName;
	}

	@Override
	protected DataSource doProcessResults(ResultSet result) throws DataAccessException {
		...
		try {
			if (result.next()) {
				...
				dataSource = new DataSource(result.getInt(c.getcDataSourcesId()), type, identifierType,
						...
						result.getDate(c.getcDataSourcesLastUpdate()), result.getBoolean(c.getcDataSourcesIsPrimary()));
			}
		} catch (Exception e) {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		if (shortName != null) {
			...
			sql.append(c.getcDataSourcesId()).append(CommandConstants.EQ).append(id);
		}
		return sql.toString();
	}
}
