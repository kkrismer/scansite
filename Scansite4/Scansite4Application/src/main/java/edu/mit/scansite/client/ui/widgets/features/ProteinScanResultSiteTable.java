...
public class ProteinScanResultSiteTable extends ScansiteWidget {
	...
	private TextColumn<ScanResultSite> motifColumn = new TextColumn<ScanResultSite>() {
		...
		public String getValue(ScanResultSite site) {
			...
			String s = (m == null || m.getDisplayName().isEmpty()) ? EMPTY_CELL_TEXT
					: m.getDisplayName();
			return s
					+ ((m.getShortName() == null || m.getShortName().isEmpty()) ? EMPTY_CELL_TEXT
							: " (" + m.getShortName() + ")");
		}
	};
					...
					dispatch.execute(new SiteInSequenceHtmlGetAction(site,
							...
							new AsyncCallback<SiteInSequenceHtmlGetResult>() {
									...
									EventBus.instance()
											.fireEvent(
													new MessageEvent(
															...
															"Displaying site in sequence failed",
															this.getClass()
																	.toString(),
															caught));
								}
								...
								public void onSuccess(
										SiteInSequenceHtmlGetResult result) {
	...
	private Column<ScanResultSite, String> scoreColumn = getColumn(
			new ClickableTextCell(), new GetValue<String>() {
				@Override
				public String getValue(ScanResultSite site) {
					if (site == null) {
				...
				public void update(int index, ScanResultSite site, String value) {
					if (results.getMotifSelection().getUserMotif() == null) {
						dispatch.execute(
								new HistogramRetrieverAction(site.getMotif()
										.getId(), results
										.getHistogramTaxonName(), results
										.getHistogramDataSourceSelection(),
										site, resultTable.getRowElement(index)
												.getAbsoluteBottom(),
										resultTable.getRowElement(index)
												.getAbsoluteLeft()),
									...
									public void onFailure(Throwable caught) {
										EventBus.instance()
												.fireEvent(
														new MessageEvent(
																MessageEventPriority.ERROR,
																"Fetching reference histogram failed",
																this.getClass()
																		.toString(),
																caught));
									}
	...
	private TextColumn<ScanResultSite> percentileColumn = new TextColumn<ScanResultSite>() {
		...
		public String getValue(ScanResultSite site) {
			...
			return ((p == null) ? EMPTY_CELL_TEXT : String.valueOf(NumberFormat
					.getFormat("0.000").format(p * 100)) + "%");
		}
	};
	...
	private Column<ScanResultSite, SafeHtml> evidenceLinkColumn = getColumn(
			new SafeHtmlCell(), new GetValue<SafeHtml>() {
				@Override
				public SafeHtml getValue(ScanResultSite site) {
					SafeHtmlBuilder shb = new SafeHtmlBuilder();
					if (site != null && site.getEvidence() != null
							&& !site.getEvidence().isEmpty()) {
				...
				@Override
				public void update(int index, ScanResultSite site,
						SafeHtml value) {
				}
			});

	private Column<ScanResultSite, SafeHtml> geneInfoColumn = getColumn(
			new SafeHtmlCell(), new GetValue<SafeHtml>() {
				@Override
				public SafeHtml getValue(ScanResultSite site) {
					SafeHtmlBuilder shb = new SafeHtmlBuilder();
					if (site == null
							|| site.getMotif().getIdentifiers() == null
							|| site.getMotif().getIdentifiers().size() == 0) {
						shb.appendEscaped("");
					} else {
						List<Identifier> identifiers = site.getMotif()
								.getIdentifiers();
	...
	private Column<ScanResultSite, String> conservedSitesColumn = getColumn(
			new ClickableTextCell(), new GetValue<String>() {
				@Override
				public String getValue(ScanResultSite site) {
				...
				public void update(int index, ScanResultSite site, String value) {
					...
					dispatch.execute(action,
							new AsyncCallback<OrthologScanMotifResult>() {
								@Override
								public void onFailure(Throwable caught) {
									waitPopup.hide();
								...
								@Override
								public void onSuccess(
										OrthologScanMotifResult result) {
									waitPopup.hide();
									hideMessage();
									EventBus.instance().fireEvent(
	...
	private Column<ScanResultSite, SafeHtml> colocalizationColumn = getColumn(
			new ClickableSafeHtmlCell(), new GetValue<SafeHtml>() {
				@Override
				public SafeHtml getValue(ScanResultSite site) {
					SafeHtmlBuilder builder = new SafeHtmlBuilder();
					...
					if (results.getProteinLocalization() == null) {
						builder.appendHtmlConstant(site.getMotifLocalization()
								.getType().getName());
						return builder.toSafeHtml();
					} else {
						if (results.getProteinLocalization().getType()
								.equals(site.getMotifLocalization().getType())) {
							builder.appendHtmlConstant("<span style=\"color: darkgreen;\">"
									+ site.getMotifLocalization().getType()
											.getName() + "</span>");
							return builder.toSafeHtml();
						} else {
				...
				@Override
				public void update(final int index, final ScanResultSite site,
						SafeHtml value) {
					dispatch.execute(
							new PredictLocalizationMotifProteinPairAction(site
									.getLocalizationDataSource(), site
									.getMotif(), site.getProtein()),
								...
								public void onSuccess(
									...
									if (result.isSuccess()) {
										...
										builder.appendHtmlConstant("<h3>Colocalization analysis - "
												+ pairResult
														.getLocalizationDataSource()
														.getDisplayName()
												+ "</h3>");
										builder.appendHtmlConstant("<h4>Motif "
												+ pairResult.getMotif()
														.getDisplayName()
												+ "</h4>");
										builder.append(displayLocalization(pairResult
												.getMotifLocalization()));
										builder.appendHtmlConstant("<h4>Protein "
												+ pairResult.getProtein()
														.getIdentifier()
												+ "</h4>");
										builder.append(displayLocalization(pairResult
												.getProteinLocalization()));

										showPopupPanel(resultTable
												.getRowElement(index)
												.getAbsoluteBottom(),
												resultTable
														.getRowElement(index)
		...
		if (localization != null) {
			...
			if (localization.getGoTerms() != null) {
				for (int i = 0; i < localization.getGoTerms().size(); ++i) {
					builder.appendEscaped(localization.getGoTerms().get(i)
							.getGoTerm().getName());
					if (localization.getGoTerms().get(i).getEvidenceCode()
							.getName() != null) {
						builder.appendHtmlConstant(" (<span title=\""
								+ localization.getGoTerms().get(i)
										.getEvidenceCode().getName()
								+ "\">"
								+ localization.getGoTerms().get(i)
										.getEvidenceCode().getCode()
								+ "</span>)");
					}
	...
	private void init() {
		...
		resultTable.addColumn(surfaceAccessColumn, "Surface Accessibility",
		...
		if (results.getProtein().getDataSource() != null) {// &&
															...
															// {
			resultTable.addColumn(evidenceLinkColumn, "Previously Mapped Site",
					"Previously Mapped Site");
		}
		...
		if (results.getLocalizationDataSource() != null) {
			resultTable.addColumn(colocalizationColumn, "Colocalization",
					"Colocalization");
		}
	...
	private void initSortableCols() {
		...
		ListHandler<ScanResultSite> sortHandler = new ListHandler<ScanResultSite>(
				list);

		sortHandler.setComparator(scoreColumn, new ScanResultSiteComparator(
				ComparableFields.SCORE));
		sortHandler.setComparator(percentileColumn,
				new ScanResultSiteComparator(ComparableFields.PERCENTILE));
		sortHandler.setComparator(motifColumn, new ScanResultSiteComparator(
				ComparableFields.MOTIF));
		sortHandler.setComparator(groupColumn, new ScanResultSiteComparator(
				ComparableFields.GROUP));
		sortHandler.setComparator(siteColumn, new ScanResultSiteComparator(
				ComparableFields.POSITION));
		sortHandler.setComparator(surfaceAccessColumn,
				new ScanResultSiteComparator(
						ComparableFields.SURFACE_ACCESSIBILITY));
		sortHandler.setComparator(sequenceColumn, new ScanResultSiteComparator(
				ComparableFields.SITE_AA));
	...
	private <C> Column<ScanResultSite, C> getColumn(Cell<C> cell,
