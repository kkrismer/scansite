 ...
 */
public class ProteinScanHandler implements ActionHandler<ProteinScanAction, ProteinScanResult> {
	...
	@Override
	public ProteinScanResult execute(ProteinScanAction action, ExecutionContext context) throws DispatchException {
		try {
			...
			if (action.getProtein().getDataSource() != null) {
				protein = factory.getProteinDao().get(action.getProtein().getIdentifier(),
						action.getProtein().getDataSource());
				if (protein == null) {
					return new ProteinScanResult("No protein with protein identifier "
							+ action.getProtein().getIdentifier() + " and data source "
							+ action.getProtein().getDataSource().getDisplayName() + " found");
				}
				// prepare annotations, for later PhosphoSite check
				// (swissprot-only)
				if (protein.getDataSource().getShortName().equalsIgnoreCase("swissprot")) {
			...
			if (accessions != null) {
				...
				for (ScanResultSite site : result.getResults().getHits()) {
					try {
						site.setEvidence(evidenceDao.getSiteEvidence(accessions, site.getSite()));
					} catch (Exception e) {
	...
	@Override
	public void rollback(ProteinScanAction action, ProteinScanResult result, ExecutionContext context)
