...
public class TaxonIdGetByDataSourceCommand extends DbQueryCommand<Set<Integer>> {
	...
	public TaxonIdGetByDataSourceCommand(Properties dbAccessConfig, Properties dbConstantsConfig,
	...
	public TaxonIdGetByDataSourceCommand(Properties dbAccessConfig, Properties dbConstantsConfig,
			boolean useTempTablesForUpdate, DataSource dataSource, String speciesRegex) {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(CommandConstants.SELECT).append(c.getcTaxaId()).append(CommandConstants.FROM)
		...
		if (speciesRegex != null && !speciesRegex.isEmpty()) {
			sql.append(connect).append(c.getcTaxaName()).append(CommandConstants.REGEXP)
