...
public class NewsDaoImpl extends DaoImpl implements NewsDao {
	 ...
	 * 
	 * @see
	 * edu.mit.scansite.server.dataaccess.NewsDao#add(edu.mit.scansite.shared
	...
	public NewsEntry get(int id) throws DataAccessException {
		...
		try {
			...
			logger.error(e.getMessage(), e);
			throw new DataAccessException("Retrieving entry from DB failed.",
					e);
		}
		return entry;
	}
	...
	public List<NewsEntry> getAll(int nrOfNewestEntries)
		...
		try {
			...
			logger.error(e.getMessage(), e);
			throw new DataAccessException("Retrieving entries from DB failed: "
					+ e.getMessage(), e);
		}
		return news;
	}
