...
public class OrthologScanSequencePatternHandler
		implements
		ActionHandler<OrthologScanSequencePatternAction, OrthologScanSequencePatternResult> {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	private final Provider<ServletContext> contextProvider;
	private final LoginHandler loginHandler;

	@Inject
	public OrthologScanSequencePatternHandler(
	...
	@Override
	public OrthologScanSequencePatternResult execute(
		...
		try {
			...
			if (action.isMainFormEmission()) {
				...
				// specified
				return feature
						.scanOrthologsBySequencePattern(action
								.getSequencePattern(), action
								.getOrthologyDataSource(), action.getProtein(),
								action.getStringency(), action
										.getAlignmentRadius(), !loginHandler
										.isSessionValidLogin(action
												.getUserSessionId()));
			} else {
				...
				if (orthologyDataSources.size() > 0) {
					...
					return feature.scanOrthologsBySequencePattern(action
							.getSequencePattern(), orthologyDataSources.get(0),
							action.getProtein(), action.getStringency(), action
									.getAlignmentRadius(), !loginHandler
									.isSessionValidLogin(action
											.getUserSessionId()));
				} else {
	...
	@Override
	public void rollback(OrthologScanSequencePatternAction action,
			OrthologScanSequencePatternResult result, ExecutionContext context)
			throws DispatchException {
	}
}
