 ...
 */
public class OrthologScanMotifHandler implements
		ActionHandler<OrthologScanMotifAction, OrthologScanMotifResult> {
	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	private final Provider<ServletContext> contextProvider;
	private final LoginHandler loginHandler;

	@Inject
	public OrthologScanMotifHandler(
		...
		try {
			...
			if (action.isMainFormEmission()) {
				...
				return feature.scanOrthologsByMotifGroup(action
						.getSitePosition(), action.getMotifGroup(), action
						.getOrthologyDataSource(), action.getProtein(), action
						.getStringency(), action.getAlignmentRadius(),
				...
				if (orthologyDataSources.size() > 0) {
					...
					return feature.scanOrthologsByMotifGroup(action
							.getSitePosition(), action.getMotifGroup(),
							orthologyDataSources.get(0), action.getProtein(),
							action.getStringency(),
							action.getAlignmentRadius(), !loginHandler
	...
	@Override
	public void rollback(OrthologScanMotifAction action,
			OrthologScanMotifResult result, ExecutionContext context)
