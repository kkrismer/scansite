...
public abstract class DbUpdater implements Runnable {
	...
	public void init(String tempDirPath, String invalidFilePrefix,
		...
		try {
			...
			try {
				this.versionFileDownloadUrl = (dataSourceMetaInfo
						.getVersionUrl() == null || dataSourceMetaInfo
						.getVersionUrl().isEmpty()) ? null : new URL(
						dataSourceMetaInfo.getVersionUrl());
			} catch (Exception e) {
			...
			throw new ScansiteUpdaterException("Can not create DaoFactory: "
					+ e.getMessage(), e);
		}
	}

	/**
	 * - download files - transliterate files to temporary scansite-specific
	 * files (extracting information) - create temporary tables - transliterate
	 * scansite-specific files to temporary table in database - update taxa in
	 * histograms - rename old table, rename temp table - clean up (remove
	 * temporary files and remove old tables)
	 * 
	 * @throws ScansiteUpdaterException
	...
	private void runUpdate() throws ScansiteUpdaterException {
		...
		try {
			...
			try {
				logger.info("updating histogram data {}", dataSourceMetaInfo
			...
			logger.info("renaming tables {}", dataSourceMetaInfo
					.getDataSource().getDisplayName());
			renameTables();
	...
	protected abstract FileTransliterator getScansiteFileTransliterator(
			String errorFilePath) throws ScansiteUpdaterException;

	/**
	 * This method is called if a version download URI is given in the
	 * configuration. What needs to be done here is to fetch the version from
	 * the given URI and return it. If null is returned, the current date is
	 * used as version name.
	 * 
	...
	/**
	 * Instantiate a file transliterator that can be used for the downloaded
	 * public database file and return it.
	 * 
	 * @param tempFilePath
	 *            the path to the temporary file.
	 * @param errorFilePath
	 *            the path to the error-file (file that contains invalid /
	 *            failed entries)
	 * @return A file transliterator that reads / parses and transliterates the
	 *         downloaded files.
	 * @throws ScansiteUpdaterException
	 */
	protected abstract FileTransliterator getDbFileTransliterator(
			String tempFilePath, String errorFilePath)
			throws ScansiteUpdaterException;

	/**
	 * Prepare the URIs of the files that have to be downloaded. This can either
	 * be a list containing the file that is given in the configuration file
	 * (most simple case), or a list of files that is determined by accessing
	 * the given URI.
	 * 
	 * @param dbDownloadUrl
	 ...
	 * @throws ScansiteUpdaterException
	 */
	protected abstract List<URL> prepareDownloadUrls(List<URL> dbDownloadUrl)
			throws ScansiteUpdaterException;
	...
	protected abstract void updateHistogramData()
			throws ScansiteUpdaterException;
	...
	private void initReader(String fileName) throws ScansiteUpdaterException {
		...
		try {
			...
			if (f.toString().endsWith(".gz")) {
				...
				if (dataSourceMetaInfo.getEncoding() != null
					...
					readers.add(new BufferedReader(new InputStreamReader(gzIn,
							dataSourceMetaInfo.getEncoding())));
				} else {
					readers.add(new BufferedReader(new InputStreamReader(gzIn)));
				}
				...
				while (zippedFile != null) {
					File newFile = new File(tempDir + File.separator + dataSourceMetaInfo.getDataSource()
							.getShortName() + "_" + zippedFile.getName());
			...
			logger.error(
					"Local file " + f + " can not be accessed: "
							+ e.getMessage(), e);
			throw new ScansiteUpdaterException("Local file " + f
	 ...
	 */
	private void downloadDbFiles(List<URL> dbDownloadUrls)
			throws ScansiteUpdaterException {
		this.dbDownloadUrls = prepareDownloadUrls(dbDownloadUrls);
		for (URL dbDownloadUrl : this.dbDownloadUrls) {
			String dbFileName = dataSourceMetaInfo.getDataSource()
					.getShortName()
					+ "_"
	...
	private void downloadFile(URL from, String to)
			throws ScansiteUpdaterException {
	 ...
	 * 
	 * @param url
	 *            The url. If "/var/www/poop.txt" is given, "poop.txt" is
	 *            returned
