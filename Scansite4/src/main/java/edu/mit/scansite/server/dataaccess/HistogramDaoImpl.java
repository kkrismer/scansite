...
public class HistogramDaoImpl extends DaoImpl implements HistogramDao {
	 ...
	 * 
	 * @see
	 * edu.mit.scansite.server.dataaccess.HistogramDao#add(edu.mit.scansite.
	 ...
	 * 
	 * @see
	 * edu.mit.scansite.server.dataaccess.HistogramDao#deleteHistograms(edu.
	 ...
	 */
	@Override
	public void deleteHistograms(Motif motif, DataSource dataSource, Taxon taxon)
	 ...
	 * 
	 * @see
	 * edu.mit.scansite.server.dataaccess.HistogramDao#getHistograms(java.util
	 * .List, edu.mit.scansite.shared.transferobjects.DataSource, int)
	 */
	@Override
	public List<ServerHistogram> getHistograms(List<Motif> motifs,
		...
		try {
			...
			for (ServerHistogram h : hists) {
				...
				if (motifs == null || motifs.isEmpty()) {
					Motif m = daoFac.getMotifDao()
							.getById(h.getMotif().getId());
					h.setMotif(m);
				}
				...
				for (ServerHistogram dh : tempHists) {
					currentDataHist = dh;
					if (h.getMotif().getId() == dh.getMotif().getId()
							&& dsId == dh.getDataSource().getId()
	 ...
	 */
	@Override
	public ServerHistogram getHistogram(int motifId, String taxonName,
	 ...
	 * 
	 * @see
	 * edu.mit.scansite.server.dataaccess.HistogramDao#updateHistogram(edu.mit
