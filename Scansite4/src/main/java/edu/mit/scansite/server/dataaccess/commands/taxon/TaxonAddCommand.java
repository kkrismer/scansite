...
public class TaxonAddCommand extends DbInsertCommand {
	...
	protected String doGetSqlStatement() throws DataAccessException {
		...
		sql.append(')').append(CommandConstants.VALUES).append('(')
				.append(CommandConstants.enquote(taxon.getName()))
				.append(CommandConstants.COMMA)
				.append((taxon.isSpecies()) ? "1" : "0");
		if (parentList != null && !parentList.isEmpty()) {
			sql.append(CommandConstants.COMMA).append('\"').append(parentList)
					.append('\"');
		}
		sql.append(')');
		return sql.toString();
