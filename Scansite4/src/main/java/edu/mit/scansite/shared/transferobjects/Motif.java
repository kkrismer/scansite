...
public class Motif extends LightWeightMotif implements IsSerializable {
	public static final char MOTIF_INFO_ID_SPLITTER = '/';
	...
	private HashMap<AminoAcid, Integer> indices = new HashMap<AminoAcid, Integer>(); // indices
																						// in
																						// aas-array
	private int[] charIndices = new int[CHAR_INDICES_SIZE];
	private double matrix[][] = new double[WINDOW_SIZE][]; // access:
															// matrix[0<position<WINDOW_SIZE][indices.get(aminoAcid)]
	 ...
	 * 
	 * @param motifString
	 *            A String generated by another motif's toString method.
	 */
	...
	private void initWithDefaults() {
		...
		for (int i = 0; i < WINDOW_SIZE; ++i) {
			...
			for (int j = 0; j < aminoAcidArray.length; ++j) {
				matrix[i][j] = (AminoAcid.isTerminal(aminoAcidArray[j])) ? DEFAULT_MATRIX_VALUE_TERMINALS
